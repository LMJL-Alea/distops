...
Let us compute the Euclidean distance matrix for the `cars` dataset.
First, we convert the dataset to a list of observations:
...
Next, we compute the Euclidean distance matrix via
\[`distops::dist()`\]:
